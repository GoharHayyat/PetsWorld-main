    ...
    const fetchDoctor = async () => {
      try {
        ...
        setFormData((prevState) => ({
          ...
          doctor_name: response.data.f_name + " " + response.data.l_name,
        }));
        setOpenBackdrop(false);
        setAvailableDays(response.data.available_days.split(","));
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const sendEmail = async () => {
    try {
      const emailData = {
        ...
        html:
          "<h1>Your Appointment request for " +
          " with Dr. " +
          doctor.f_name +
          " " +
          doctor.l_name +
          " on " +
          formData.datereq +
          " is successfully sent. We will contact you shortly.</h1>",
      };
  ...
  const handleSubmit = async (event) => {
    ...
    if (formData) {
      ...
      if (availableDays.includes(weekday)) {
        ...
        setOpenBackdrop(false);
        toast.warn("Available days are: " + doctor.available_days);
      }
    } else {
      toast.error("Booked");
      setOpenBackdrop(false);
                ...
                >
                  ...
                  <div id="div_block-28-144" class="ct-div-block">
                    ...
                    >
                      ...
                      <br />
                      {/* <p style={{ color: "red" }}>
                        *Check this box if you want to get appointment for
                        Vaccination
                      </p>
                      <div
                        style={{ display: "flex", justifyContent: "center" }}
                      ...
                      {isOtherSelected ? null : (
                        <div>
                          <p style={{ color: "red" }}>
                            *Check this box if you want to get an appointment
                            for Vaccination
                          </p>
                          <div
                            style={{
